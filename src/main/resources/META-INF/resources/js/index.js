...
document.addEventListener('DOMContentLoaded', async () => {
  document.getElementById('zmo-searchMainPage')?.addEventListener('submit', ignoreEmptyFieldsOnSubmit);
  const input = document.getElementById('zmo-searchInput');
  if (input) {
    try {
      const placeholder = await getTranslation(currentLang, 'zmo.index.search.placeholder');
